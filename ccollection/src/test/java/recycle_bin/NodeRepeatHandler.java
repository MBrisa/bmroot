...
import org.mbrisa.ccollection.MTreeBuilder;
...
public interface NodeRepeatHandler<E> {
	...
	/**
	 * 如果 {@link #repeatable()} 返回 false，那么在向 {@link MTreeBuilder} 中添加节点时将忽略该方法的调用。
	 * 如果 {@link #repeatable()} 返回 true, 那么该方法需要返回参数 parent 的一个下标，该下标所指的对象将作为参数 current 的父节点。如果返回 -1 则 {@link MTreeBuilder} 将不会将不会对 current 进行添加
	 * @param parent
	 * @param child
	 * @return
